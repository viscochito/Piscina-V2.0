 ...
 * 
 * VERSIÓN 1.0 - Copia de seguridad
 * Fecha: 2025
 * 
 * Esta es una copia de seguridad del formato y estilo del PDF actual.
 * Se guarda para poder volver a esta versión si es necesario.
 */
...
async function generarPDFBasico(
  ...
  // Inferir tipo de piscina basado en dimensiones
  const tipoPiscina = dim.largo === dim.ancho ? 'Cuadrada' : 'Rectangular';
  const descripcionTipo = `${tipoPiscina}, ${tipoTrabajoTexto}`;
